...
When established Rshiny dashboard, we encountered problems of decision of choosing appropriate interaction plots, setting indexes options and converting different types of plots. Our biggest difficulty was previewing shiny dashborad in website, since `rmarkdown::render("Rshiny.Rmd", output_format = "flexdashboard::flex_dashboard")` conflicts original html document's format.

## Insights

We still need more data to support our conclusion, since our original dataset contains too many NA values. Although we used mean value to represent missing values, information bias still exist, affecting  accuracy of conclusion.